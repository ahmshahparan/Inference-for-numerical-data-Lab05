...
```
It shows us that the two factors are comparable. However nonsmokers have a much higher variability.
...
non-smoking mothers.
```{r}
inference(y = nc$weight, x = nc$habit, est = "mean", type = "ci", null=0,alternative="twosided",method = "simulation")
```
...
```
P-value is 2. Average mother’s weight gain is dependent of age. We reject the null in favor of the alternative hypothesis.
...
```
It appears from the historgram as well as looking for the max age of a younger mom and the minimum age of a mature mom that the age cut off for the younger mom and mature mom group is: 34 years old and younger for young mom and 35 years old and older for mature mom.
...
```
I have tested whether or not the average weight of a mother was dependent on her marital status. The probability that it was independent was 0 (p-value) that means we were in favor of the alternative hypothesis, and the average is dependent.
